...
export class PeriodComponent implements OnInit {
  ...
  editable: boolean;
  periodArray: PeriodElement[];
  displayedColumns: string[] = ['period_id', 'period_name', 'period_gender'];
  dataSource: any;
  ...
  init() {
    this.numberOfPeriod = AppSettings.getMalePeriods() + AppSettings.getFemalePeriods();
    this.numberOfMale = AppSettings.getMalePeriods();
    this.numberOfFemale = AppSettings.getFemalePeriods();
    this.periodArray = AppSettings.getPeriodArray();
    this.editable = false;

    this.dataSource = new MatTableDataSource<PeriodElement>(this.periodArray);
    this.dataSource.paginator = this.paginator;
  }

  editNumber() {
    this.tempPeriod = this.numberOfPeriod;
    this.tempMale = this.numberOfMale;
    this.tempFemale = this.numberOfFemale;
    this.editable = true;
  }
  updateNumber() {
    this.numberOfMale = Math.floor(this.tempMale);
    this.numberOfFemale = Math.floor(this.tempFemale);
    this.numberOfPeriod = this.numberOfMale + this.numberOfFemale;
    this.editable = false;
    AppSettings.setMalePeriods(this.numberOfMale);
    AppSettings.setFemalePeriods(this.numberOfFemale);
  }
  exportData() {
    var options = {
      fieldSeparator: ',',
    ...
    new Angular5Csv(this.periodArray, 'Period Data', options);
  }
  importData() {
    this.editable = false;
    setTimeout(function () {
  ...
  extractData(data) { // Input csv data to the function
    ...
    for (let i = 0; i < allTextLines.length; i++) {
      ...
      if (data.length == headers.length) {
        if (i == 0) {
          ...
          this.periodArray.push(tobj);
          (tobj["period_gender"].toLowerCase().indexOf("female") > -1) ? female++ : male++;
        }
      }
    }
    ...
    this.numberOfPeriod = male + female;
    this.dataSource = new MatTableDataSource<PeriodElement>(this.periodArray);
    this.dataSource.paginator = this.paginator;

    AppSettings.setMalePeriods(this.numberOfMale);
    AppSettings.setFemalePeriods(this.numberOfFemale);
    AppSettings.setPeriodArray(this.periodArray);
  }
  generatePeriod() {
    this.periodArray = [];
    for (let i = 0; i < this.numberOfPeriod; i++) {
      let obj = { period_id: "", period_name: "", period_gender: "" };
      if (i < this.tempMale)
        obj['period_gender'] = "Male";
      else
        obj['period_gender'] = "Female";
      obj['period_id'] = this.generatePeriodId();
      obj['period_name'] = this.generateName();
      this.periodArray.push(obj);
    }
    this.periodArray.sort(this.compareSecondColumn);
    this.dataSource = new MatTableDataSource<PeriodElement>(this.periodArray);
    this.dataSource.paginator = this.paginator;
  ...
  generateName() {
    let id = Math.floor(Math.random() * periodNames.length);
    return periodNames[id];
  }
  generatePeriodId() {
    let id = "";
    for (let i = 0; i < 12; i++) {
      var random_boolean = Math.random() >= 0.5;
      id += random_boolean ? "1" : "0";
    }
    return id;
  }
  compareSecondColumn(a, b) {
    if (a['period_id'] === b['period_id']) {
      ...
      return (a['period_id'] < b['period_id']) ? -1 : 1;
    }
  }
}
