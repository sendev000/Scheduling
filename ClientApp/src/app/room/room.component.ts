...
export class RoomComponent implements OnInit {
  ...
  editable: boolean;
  roomArray: RoomElement[];
  displayedColumns: string[] = ['room_id', 'room_name', 'room_gender'];
  dataSource: any;
  ...
  init() {
    this.numberOfRoom = AppSettings.getMaleRooms() + AppSettings.getFemaleRooms();
    this.numberOfMale = AppSettings.getMaleRooms();
    this.numberOfFemale = AppSettings.getFemaleRooms();
    this.roomArray = AppSettings.getRoomArray();
    this.editable = false;

    this.dataSource = new MatTableDataSource<RoomElement>(this.roomArray);
    this.dataSource.paginator = this.paginator;
  }

  editNumber() {
    this.tempRoom = this.numberOfRoom;
  ...
  updateNumber() {
    ...
    this.numberOfRoom = this.numberOfMale + this.numberOfFemale;
    this.editable = false;
    AppSettings.setMaleRooms(this.numberOfMale);
    AppSettings.setFemaleRooms(this.numberOfFemale);
  }
  exportData() {
    var options = {
      fieldSeparator: ',',
    ...
    new Angular5Csv(this.roomArray, 'Room Data', options);
  }
  importData() {
    this.editable = false;
    setTimeout(function () {
  ...
  extractData(data) { // Input csv data to the function
    ...
    for (let i = 0; i < allTextLines.length; i++) {
      ...
      if (data.length == headers.length) {
        if (i == 0) {
          ...
          this.roomArray.push(tobj);
          (tobj["room_gender"].toLowerCase().indexOf("female") > -1) ? female++ : male++;
        }
      }
    }
    ...
    this.numberOfRoom = male + female;
    this.dataSource = new MatTableDataSource<RoomElement>(this.roomArray);
    this.dataSource.paginator = this.paginator;

    AppSettings.setMaleRooms(this.numberOfMale);
    AppSettings.setFemaleRooms(this.numberOfFemale);
    AppSettings.setRoomArray(this.roomArray);
  }
  generateRoom() {
    this.roomArray = [];
    for (let i = 0; i < this.numberOfRoom; i++) {
      let obj = { room_id: "", room_name: "", room_gender: "" };
      if (i < this.tempMale)
        obj['room_gender'] = "Male";
      else
        obj['room_gender'] = "Female";
      obj['room_id'] = this.generateRoomId();
      obj['room_name'] = this.generateName();
      this.roomArray.push(obj);
    }
    this.roomArray.sort(this.compareSecondColumn);
    this.dataSource = new MatTableDataSource<RoomElement>(this.roomArray);
    this.dataSource.paginator = this.paginator;
  ...
  generateName() {
    let id = Math.floor(Math.random() * roomNames.length);
    return roomNames[id];
  }
  generateRoomId() {
    let id = "";
