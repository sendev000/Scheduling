...
export class SchoolComponent implements OnInit {
  ...
  editable: boolean;
  schoolArray: SchoolElement[];
  displayedColumns: string[] = ['school_id', 'school_name', 'school_gender'];
  dataSource: any;
  ...
  init() {
    this.numberOfSchool = AppSettings.getMaleSchools() + AppSettings.getFemaleSchools();
    this.numberOfMale = AppSettings.getMaleSchools();
    this.numberOfFemale = AppSettings.getFemaleSchools();
    this.schoolArray = AppSettings.getSchoolArray();
    this.editable = false;

    this.dataSource = new MatTableDataSource<SchoolElement>(this.schoolArray);
    this.dataSource.paginator = this.paginator;
  }

  editNumber() {
    this.tempSchool = this.numberOfSchool;
  ...
  updateNumber() {
    ...
    this.numberOfSchool = this.numberOfMale + this.numberOfFemale;
    this.editable = false;
    AppSettings.setMaleSchools(this.numberOfMale);
    AppSettings.setFemaleSchools(this.numberOfFemale);
  }
  exportData() {
    var options = {
      fieldSeparator: ',',
    ...
    new Angular5Csv(this.schoolArray, 'School Data', options);
  }
  importData() {
    this.editable = false;
    setTimeout(function () {
  ...
  extractData(data) { // Input csv data to the function
    ...
    for (let i = 0; i < allTextLines.length; i++) {
      ...
      if (data.length == headers.length) {
        if (i == 0) {
          ...
          this.schoolArray.push(tobj);
          (tobj["school_gender"].toLowerCase().indexOf("female") > -1) ? female++ : male++;
        }
      }
    }
    ...
    this.numberOfSchool = male + female;
    this.dataSource = new MatTableDataSource<SchoolElement>(this.schoolArray);
    this.dataSource.paginator = this.paginator;

    AppSettings.setMaleSchools(this.numberOfMale);
    AppSettings.setFemaleSchools(this.numberOfFemale);
    AppSettings.setSchoolArray(this.schoolArray);
  }
  generateSchool() {
    this.schoolArray = [];
    for (let i = 0; i < this.numberOfSchool; i++) {
      let obj = { school_id: "", school_name: "", school_gender: "" };
      if (i < this.tempMale)
        obj['school_gender'] = "Male";
      else
        obj['school_gender'] = "Female";
      obj['school_id'] = this.generateSchoolId();
      obj['school_name'] = this.generateName();
      this.schoolArray.push(obj);
    }
    this.schoolArray.sort(this.compareSecondColumn);
    this.dataSource = new MatTableDataSource<SchoolElement>(this.schoolArray);
    this.dataSource.paginator = this.paginator;
  ...
  generateName() {
    let id = Math.floor(Math.random() * schoolNames.length);
    return schoolNames[id];
  }
  generateSchoolId() {
    let id = "";
    for (let i = 0; i < 12; i++) {
  ...
  compareSecondColumn(a, b) {
    if (a['school_id'] === b['school_id']) {
      ...
      return (a['school_id'] < b['school_id']) ? -1 : 1;
    }
  }
}
