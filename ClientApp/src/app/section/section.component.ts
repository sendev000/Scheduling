...
export class SectionComponent implements OnInit {
  ...
  editable: boolean;
  sectionArray: SectionElement[];
  displayedColumns: string[] = ['section_id', 'section_name', 'section_gender'];
  dataSource: any;
  ...
  init() {
    this.numberOfSection = AppSettings.getMaleSections() + AppSettings.getFemaleSections();
    this.numberOfMale = AppSettings.getMaleSections();
    this.numberOfFemale = AppSettings.getFemaleSections();
    this.sectionArray = AppSettings.getSectionArray();
    this.editable = false;

    this.dataSource = new MatTableDataSource<SectionElement>(this.sectionArray);
    this.dataSource.paginator = this.paginator;
  }

  editNumber() {
    this.tempSection = this.numberOfSection;
  ...
  updateNumber() {
    ...
    this.numberOfSection = this.numberOfMale + this.numberOfFemale;
    this.editable = false;
    AppSettings.setMaleSections(this.numberOfMale);
    AppSettings.setFemaleSections(this.numberOfFemale);
  }
  exportData() {
    var options = {
      fieldSeparator: ',',
    ...
    new Angular5Csv(this.sectionArray, 'Section Data', options);
  }
  importData() {
    this.editable = false;
    setTimeout(function () {
  ...
  extractData(data) { // Input csv data to the function
    ...
    for (let i = 0; i < allTextLines.length; i++) {
      ...
      if (data.length == headers.length) {
        if (i == 0) {
          ...
          this.sectionArray.push(tobj);
          (tobj["section_gender"].toLowerCase().indexOf("female") > -1) ? female++ : male++;
        }
      }
    }
    ...
    this.numberOfSection = male + female;
    this.dataSource = new MatTableDataSource<SectionElement>(this.sectionArray);
    this.dataSource.paginator = this.paginator;

    AppSettings.setMaleSections(this.numberOfMale);
    AppSettings.setFemaleSections(this.numberOfFemale);
    AppSettings.setSectionArray(this.sectionArray);
  }
  generateSection() {
    this.sectionArray = [];
    for (let i = 0; i < this.numberOfSection; i++) {
      let obj = { section_id: "", section_name: "", section_gender: "" };
      if (i < this.tempMale)
        obj['section_gender'] = "Male";
      else
        obj['section_gender'] = "Female";
      obj['section_id'] = this.generateSectionId();
      obj['section_name'] = this.generateName();
      this.sectionArray.push(obj);
    }
    this.sectionArray.sort(this.compareSecondColumn);
    this.dataSource = new MatTableDataSource<SectionElement>(this.sectionArray);
    this.dataSource.paginator = this.paginator;
  ...
  generateName() {
    let id = Math.floor(Math.random() * sectionNames.length);
