...
export class SectionComponent implements OnInit {
  ...
  updateContent(){
    let scheduleData=AppSettings.getScheduleData();
    let tempData={}, count;
    let periodHash={}, semesterHash={}, hashPeriodSemester={};
    ...
    for(let i=0;i<scheduleData.length;i++){
      ...
      if(this.selectCourse.length!=0 && this.selectCourse!=eObj['course_section'].split("-", 1))
        check=false;

      if(check){
        let key=eObj['course_section']+"|||"+eObj['term']+"|||"+
                eObj['course_grade']+"|||"+eObj["semester"].toString()+"|||"+
