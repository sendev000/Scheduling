...
export class CourseComponent implements OnInit {
  ...
  editable: boolean;
  courseArray: CourseElement[];
  displayedColumns: string[] = ['course_id', 'course_name', 'course_gender'];
  dataSource: any;
  ...
  init() {
    this.numberOfCourse = AppSettings.getMaleCourses() + AppSettings.getFemaleCourses();
    this.numberOfMale = AppSettings.getMaleCourses();
    this.numberOfFemale = AppSettings.getFemaleCourses();
    this.courseArray = AppSettings.getCourseArray();
    this.editable = false;

    this.dataSource = new MatTableDataSource<CourseElement>(this.courseArray);
    this.dataSource.paginator = this.paginator;
  }

  editNumber() {
    this.tempCourse = this.numberOfCourse;
  ...
  updateNumber() {
    ...
    this.numberOfCourse = this.numberOfMale + this.numberOfFemale;
    this.editable = false;
    AppSettings.setMaleCourses(this.numberOfMale);
    AppSettings.setFemaleCourses(this.numberOfFemale);
  }
  exportData() {
    var options = {
      fieldSeparator: ',',
    ...
    new Angular5Csv(this.courseArray, 'Course Data', options);
  }
  importData() {
    this.editable = false;
    setTimeout(function () {
  ...
  extractData(data) { // Input csv data to the function
    ...
    for (let i = 0; i < allTextLines.length; i++) {
      ...
      if (data.length == headers.length) {
        if (i == 0) {
          ...
          this.courseArray.push(tobj);
          (tobj["course_gender"].toLowerCase().indexOf("female") > -1) ? female++ : male++;
        }
      }
    }
    ...
    this.numberOfCourse = male + female;
    this.dataSource = new MatTableDataSource<CourseElement>(this.courseArray);
    this.dataSource.paginator = this.paginator;

    AppSettings.setMaleCourses(this.numberOfMale);
    AppSettings.setFemaleCourses(this.numberOfFemale);
    AppSettings.setCourseArray(this.courseArray);
  }
  generateCourse() {
    this.courseArray = [];
    for (let i = 0; i < this.numberOfCourse; i++) {
      let obj = { course_id: "", course_name: "", course_gender: "" };
      if (i < this.tempMale)
        obj['course_gender'] = "Male";
      else
        obj['course_gender'] = "Female";
      obj['course_id'] = this.generateCourseId();
      obj['course_name'] = this.generateName();
      this.courseArray.push(obj);
    }
    this.courseArray.sort(this.compareSecondColumn);
    this.dataSource = new MatTableDataSource<CourseElement>(this.courseArray);
    this.dataSource.paginator = this.paginator;
  ...
  generateName() {
    let id = Math.floor(Math.random() * courseNames.length);
    return courseNames[id];
  }
  generateCourseId() {
    let id = "";
    for (let i = 0; i < 12; i++) {
