...
export class StudentComponent implements OnInit {
  ...
  updateContent() {
    ...
    for (let each in tempData) {
      ...
      this.studentObj[indR + 1][indSP + 1] = tempData[each];
    }

    count = this.studentObj.length;
    // count = 2;
    this.specialStudent = [];
    for (let i = 1; i < count; i++) {
      let obj = this.studentObj[i],
        emptyCount = 0;
      for (let j = 1; j < this.maxColumn; j++) {
        let arr = obj[j];
        if (arr && arr.length > 1)
        {
          this.specialStudent.push({
            "Student" : this.studentObj[i][0][0], 
            "Semester-Period" : this.studentObj[0][j][0]
          });
        }
        if (arr && arr.length > 0) {
          let firstLine = '';
          ...
          for (let k = 0; k < arr.length; k++) {
            ...
            cs[section].push([ arr[k]['teacher_name'], arr[k]['room_name'] ]);
          }
          const ordered = {};
          Object.keys(cs)
            .sort()
          ...
          if (this.sp_color==true)
            obj[j] = [firstLine, this.getColor(arr.length), arr.length];
          else
            obj[j] = [firstLine, "#FFFFFF", arr.length];
    ...
    // console.log(this.specialStudent);
    // console.log(this.studentObj);
    this.totalAnalysis = this.studentObj.length - 1;
  ...
  exportData() {
    ...
    for (let i = 1; i < count; i++) {
      ...
      for (let j = 0; j < this.maxColumn; j++) {
        key = this.studentObj[0][j][0];
        let rr = this.studentObj[i][j][0].split(search);
        if (j==0){
  ...
  // editable: boolean;
  // studentArray: StudentElement[];
  // displayedColumns: string[] = ['student_id', 'student_name', 'student_gender'];
  // dataSource: any;
  ...
  // editNumber() {
  //   this.tempStudent = this.numberOfStudent;
  ...
  //   };
  //   new Angular5Csv(this.studentArray, 'Student Data', options);
  // }
  // importData() {
  //   this.editable = false;
  //   setTimeout(function () {
  ...
  //         }
  //         this.studentArray.push(tobj);
  //         (tobj["student_gender"].toLowerCase().indexOf("female") > -1) ? female++ : male++;
  //       }
  //     }
  //   }
  ...
  //     let obj = { student_id: "", student_name: "", student_gender: "" };
  //     if (i < this.tempMale)
  //       obj['student_gender'] = "Male";
  //     else
  //       obj['student_gender'] = "Female";
  //     obj['student_id'] = this.generateStudentId();
  //     obj['student_name'] = this.generateName();
